...
- [<i class="fab fa-markdown"></i> `Download Markdown`](/homeworks/03-problem-set.Rmd)
...
Make a histogram of the residuals. Do they look roughly normal?
...
Are any results statistically significant or not? 
...
Use the `infer` package to simulate 1000 additional samples^[`generate(reps = 1000, type = "permute")`] to calculate the slope^[`calculate(stat = "slope")`] under the null hypothesis that there is no connection between attendance and runs.^[`hypothesize(null = "independence")`], and save this as an object (it's a `tibble`). 

### Part L
Save the (value of) our sample $\hat{\beta_1}$ from your regression in Part C as an object. Make a histogram of the simulated slopes, and plot our sample slope on that histogram, shading the $p$-value.^[You can use `ggplot2` to plot a histogram in the normal way and add a `geom_vline()`, setting `xintercept` equal to your saved object with the sample $\hat{\beta_1}$ value. *Alternatively*, you can use `infer` to pipe your `tibble` of simulations into `visualize()`, and inside `visualize()` set `obs_stat` equal to your saved $\hat{\beta_1}$ object. Regardless of which method you use, then add `+shade_p_value()`. Inside this, set `obs_stat` equal to your saved slope, and add `direction = "two_sided"`.]

### Part M
Get the confidence interval for your slope estimate,^[`tidy()` your original regression, with `, conf.int = TRUE` inside the command, then `select(conf.low, conf.high)` and `filter` by your `x` variable. Save this as an object.] and then make a histogram of the simulated slopes (like part L), but instead, add `+shade_confidence_interval()`.^[Inside of this, set `endpoints` equal to the object you just made with the low and high confidence interval values.]
