...
- ii. Write at least one testable implication of the model.^[i.e. what variable(s) should be independent of another? or what should be independent of another *once* controlling for other variables?]
- iii. Identify which variable(s) need to be controlled for to estimate the causal effect of `X` on `Y`.
- iv. Write out the regression equation (abstractly) for properly identifying the causal effect, based on part iii.
...
library(corrplot)
bwght %>%
  select(bwght, cigs, faminc, cigprice, motheduc) %>%
...
- For *conditional* independencies $(x \per y | z, a)$: run a regression of $y$ on $x, z, a$.
...
```{r}
lm(motheduc~cigprice, data = bwght) %>% summary()
```

```{r}
lm(faminc~cigprice, data = bwght) %>% summary()
```
...
## Question 11
Test these implications appropriately, like you did in Question 5. 

```{r}
lm(bwght~cigprice+cigs+motheduc+faminc, data = bwght) %>% summary()
```
