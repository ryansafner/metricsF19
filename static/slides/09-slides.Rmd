...
- We will begin with .whisper[bivariate] data for relationships between $X$ and $Y$
- Immediate aim is to explore .onfire[associations] between variables, quantified with .shout[correlation] and .shoutcaps[linear regression]
...
- Your Occasional Reminder: .shoutcaps[Correlation does not imply causation!]
...
- $\beta_{ClassSize}$ is the .onfire[slope] of the regression line
...
M <-cor(mtcars) # need to make a correlation matrix with cor(); can only include numeric variables
...
- The solution to the SSE minimization problem yields:.footnote[See **Class Info Page** for proofs.]
