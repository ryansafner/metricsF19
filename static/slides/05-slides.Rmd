...
.center[
![:scale 90%](https://www.dropbox.com/s/n9fm2lkii5renzf/messyexcel.png?raw=1)
]
]

.pull-right[
.center[
![](https://www.dropbox.com/s/gp1lxciqaxq141q/datawrangling0.jpg?raw=1)
]
]
---
...
.pull-left[
.center[
.polaroid[![](https://www.dropbox.com/s/zke6m65wp3upouj/workflow.png?raw=1)]
...
.pull-right[
- Consider Hadley Wickham's famous diagram:
1. .whisper[Import] raw data from out there in the world
2. .whisper[Tidy] it into a form that you can use 
3. .whisper[Explore] the data (do these 3 repetitively!)
    - **Transform**
    - **Visualize**
    - **Model**
4. .whisper[Communicate] results to target audience
...
.center[
![:scale 80%](https://www.dropbox.com/s/9h1trto0iapnklx/tidyverse2.png?raw=1)
]
---
...
.center[
![:scale 100%](https://www.dropbox.com/s/74srdjrw13mgx4x/magrittr.png?raw=1)
]
]

.right-column[

- The `magrittr` package allows us to use the **"pipe" operator** (`%>%`)<sup>.red[1]</sup>
...
.center[
![:scale 60%](https://www.dropbox.com/s/rl1sj67y2m8fqy5/readxl.png?raw=1)

![:scale 60%](https://www.dropbox.com/s/ua6t5wz1se34vmo/haven.png?raw=1)]
]
...
- `Base R` grammar: passively running functions on nouns: `function(object)`
- `dplyr` grammar: actively using verbs: `dplyr_verb(df, conditions)`<sup>.red[1]</sup>
...
.footnote[<sup>.red[1]</sup> With the pipe, even simpler: `df %>% dplyr_verb(conditions)`]
...
.center[
![](https://www.dropbox.com/s/n9o9x29mlvyrr8q/filter.png?raw=1)
]
...
- In computer code, you are selecting only observations for which a set of logical conditions are `TRUE`<sup>.red[1]</sup>
...
.footnote[<sup>.red[1]</sup> See `?Comparison` and `?Base::Logic`.]
...
.center[
![](https://www.dropbox.com/s/meq9tyvn4u5wcfb/select.png?raw=1)
]
...
.center[
![](https://www.dropbox.com/s/1ktrsvya8pq3hh3/mutate.png?raw=1)
]
...
.center[
![](https://www.dropbox.com/s/d7hiv0fst5nhhzu/summarize.png?raw=1)
]
...
- `summarize`<sup>.red[1]</sup> outputs a tibble of desired summary statistics
...
.footnote[<sup>.red[1]</sup> Also the more civilised non-U.S. English spelling `summarise` also works. `dplyr` was written by a Kiwi after all!
]
---
...
.center[
![](https://www.dropbox.com/s/pu5stir0u59ayae/tidyr2.png?raw=1)
]
---
...
- .shout["tidy" data]<sup>.red[1]</sup> are (an opinionated view of) data where

1. Each .hl[variable] is in a .hl[column]
2. Each .hl[observation] is a .hl[row]
3. Each .hl[observational unit] forms a .hl[table]<sup>.red[2]</sup>
...
.footnote[<sup>.red[1]</sup> This is the core and eponym of the `tidyverse`: all associated packages and functions use or require this data format!

<sup>.red[2]</sup> Alternatively, sometimes rule 3 is "every value is its own cell."]
---
...
.center[
![](https://www.dropbox.com/s/ukxnk8y9be3fyeb/tidydata1.png?raw=1)
]
...
- **Common source of "un-tidy" data**: .hl[Column headers are values, not variable names]
...
- **Another common source of "un-tidy" data**: .hl[observations are scattered across multiple rows]
...
.pull-left[
.center[
![](https://www.dropbox.com/s/buiczvm2wny72xz/tidyr-spread-gather.gif?raw=1)
]
]
...
.center[
![](https://www.dropbox.com/s/99sppuxkb3hdfx3/joinfull.png?raw=1)
]
---
...
.footnote[<sup>.red[1]</sup> Note the columns must be identical between the original dataset and the new observations

<sup>.red[2]</sup> Note the rows must be identical between original dataset and new variable]
...
![](https://www.dropbox.com/s/b1xq2n5t61koxvo/bindcols.png?raw=1)
]
]
...
![](https://www.dropbox.com/s/4wnon47pi6m19z6/bindrows.png?raw=1)
]
]
...
.center[
![](https://www.dropbox.com/s/3ao0g9yfwg46mdw/dplyr-joins.png?raw=1)
