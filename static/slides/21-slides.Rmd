...
- The .red[exponential function], $e^x$ or **$exp(x)$**, where base $e=2.71828...$
...
- An .shout[elasticity] between two variables, $E_{Y,X}$ describes the .whisper[responsiveness] of one variable to a change in another. 
...
&=\cfrac{\frac{\Delta y}{y}}{\frac{\Delta x}{x}}\\
\end{align*}$$
...
\beta_1&=\cfrac{\big(\frac{\Delta Y}{Y}\big)}{X}\\
\end{align*}$$
...
$$\widehat{\text{ln(Life Expectancy)}}_i=2.864+0.0147 \, \text{ln(GDP)}_i$$
...
- $F$ is an .whisper[analysis of variance (ANOVA)]
...
- $R^_r$: the $R^2$ from the **restricted model** (null hypothesis)
]
...
- .whisper[Key takeaway]: The bigger the difference between $(R^2_u-R^2_r)$, the greater the improvement in fit by adding variables, the larger the $F$!
