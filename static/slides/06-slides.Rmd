...
.pull-right[
.center[
![](https://www.dropbox.com/s/czklhgt5llrekjc/newrmarkdown.png?raw=1)
]
]
...
- You can add [additional options](this guide](https://yihui.name/knitr/options/)) inside the `{braces}` after `r`, some common options:
...
- First term after the `{` is your **citation key** (e.g. `safner2016`)
]
---
...
- Markdown files are [**plain text**](https://en.wikipedia.org/wiki/Plain_text) files and can be edited in *any* text editor
    ...
    - many good [text editors](https://en.wikipedia.org/wiki/Text_editor) out there
...
.footnote[Big shout outs also to [Typora](https://www.typora.io/) and [Ulysses](https://ulysses.app/) (Mac only) that both write (and preview) Markdown in a simple interface, no distractions.]
...
- Honestly, I write **everything** in R Studio's text editor
    - Syntax highlighting
    - Actually can *run* R code, autocomplete, etc<sup>.red[1]</sup>
    - Can render the markdown to an output format: html, pdf, etc.

.footnote[<sup>.red[1]</sup> You can *write* R code in other text editors, but you can't *execute* them outside of *R Studio* (or the command line, but that's too advanced.) Same with actually rendering your markdown to an output (pdf, html, etc)]
...
# knitr: What's Happening On Your Computer
...
.center[
![:scale 80%](https://www.dropbox.com/s/i1ijatt56ncxi0e/github%202.png?raw=1)
...
.center[
![:scale 70%](https://www.dropbox.com/s/egfojonn2s54dk2/basic-remote-workflow.png?raw=1)
]
