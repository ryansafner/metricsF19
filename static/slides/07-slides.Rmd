...
- .hl[Categorial data] place an individual into one of several possible *categories*
...
- See today's class notes for more about the summation operator, $\displaystyle\Sigma$, it'll come up again!
...
- Points *beyond* whiskers are a common definition of **outliers**
]
.pull-right[
...
```{r, echo=T}
quizzes_new %>% summary(quiz_1)

quizzes_new %>% summary(quiz_2)
```
]
...
```{r, echo=T}
# loads .R files to full functions from; make sure it's in working directory/Project!
source("../files/summaries.R") 

# let's summarize "hwy" and "cty" from mpg dataset
mpg %>%
...
- Every observation .onfire[deviates] from the mean of the data: 
$$deviation_i = x_i-\mu	$$
...
- The .shout[population variance `\\((\sigma^2)\\)`] of a *population* distribution measures the average of the *squared* deviations from the *population* mean
...
- The .shout[sample variance `\\((s^2)\\)`] of a *sample* distribution measures the average of the *squared* deviations from the *sample* mean

$$\sigma^2 = \frac{1}{n-1}\displaystyle\sum^n_{i=1} (x_i-\mu)^2$$
...
$$s=\sqrt{s^2}=\sqrt{\frac{1}{n-1}\displaystyle\sum^n_{i=1} (x_i-\mu)^2	}$$
