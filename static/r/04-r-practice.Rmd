...
## 10. Now let's try adding some color, specifically, add an `aes`thetic that maps `continent` to `color.` 
...
## 12. Redo your plot from 10 for `lifeExp` instead of `gdpPercap`.
...
## 22. Now let's try subsetting by looking only at North America. Take the `gapminder` dataframe and subset it to only look at `continent=="Americas"`). Assign this to a new dataframe object (call it something like `america`.) Now, use *this* as your `data`, and redo the graph from question 16. (You might want to take a look at your new dataframe to make sure it worked first!)

## 23. Try this again for the *whole* world, but just for observations in the year 2000. 